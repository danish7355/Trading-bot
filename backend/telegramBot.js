...
async function sendTPAlert(trade, tpNumber, closePrice, pnlAmount) {
  ...
  const pnlPct = ((pnlAmount / trade.positionValue) * 100).toFixed(2);
...
Realized (this close): +$${pnlAmount.toFixed(2)} (+${pnlPct}%)
...
━━━━━━━━━━━━━━━━━━━━━
Time: ${formatUTCDateTime(Date.now())}
SL Price: $${closePrice.toFixed(2)}
Loss: -$${lossAbs.toFixed(2)} (-${lossPct}%)
Score at close: ${trade.scoreAtExit || '?'}/100`;
...
module.exports = {
  sendMessage,
  sendSignalAlert,
  sendWMConfirmedAlert,
  sendTradeOpenedAlert,
  sendTPAlert,
  sendSLAlert,
  sendTrailingMovedAlert,
  sendDailyLimitAlert,
  sendRangingAlert,
  sendTestAlert
};
